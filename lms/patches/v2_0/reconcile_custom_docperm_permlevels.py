...
"""Copy each LMS doctype's shipped permlevel > 0 DocPerm rows into its Custom DocPerm
set wherever they are missing there, so a doctype an admin has customised keeps the
...
def execute():
	for doctype, rows in get_shadowed_permlevel_rows().items():
		inserted = False
		for role, permlevel in rows:
