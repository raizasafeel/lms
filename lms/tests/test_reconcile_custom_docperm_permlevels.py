...
from lms.lms.docperm_shadow import warn_about_shadowed_permlevels
from lms.patches.v2_0.reconcile_custom_docperm_permlevels import execute
...
class TestReconcileCustomDocPermPermlevels(IntegrationTestCase):
	"""lms.patches.v2_0.reconcile_custom_docperm_permlevels re-adds, to a doctype's
	Custom DocPerm set, the shipped permlevel > 0 DocPerm rows that set has shadowed
	(frappe.model.meta.Meta.set_custom_permissions replaces a doctype's permissions
	wholesale the moment it has any Custom DocPerm row -- see lms/lms/docperm_shadow.py).
	The shipped rows are read from tabDocPerm, the table that replacement never touches."""
	...
	def setUp(self):
		...
		# What every test that runs execute() against LMS Assignment must clean up:
		# execute() itself inserts Custom DocPerm rows we never named, so cleanup is by
		# snapshot-diff rather than by tracked name.
		self.assignment_custom_perm_baseline = set(
			frappe.get_all("Custom DocPerm", filters={"parent": ASSIGNMENT_DOCTYPE}, pluck="name")
		)
	...
	def tearDown(self):
		...
		leaked = (
			set(frappe.get_all("Custom DocPerm", filters={"parent": ASSIGNMENT_DOCTYPE}, pluck="name"))
			- self.assignment_custom_perm_baseline
		)
		for name in leaked:
			if frappe.db.exists("Custom DocPerm", name):
				# nosemgrep: lms-unjustified-ignore-permissions - removes a row this test (or execute() under test) added
	...
	def test_moderator_regains_the_model_answer(self):
		...
		shipped permlevel-1 rows regains it once execute() reconciles that snapshot.
	...
	def test_second_run_inserts_nothing(self):
		"""Idempotence: once execute() has reconciled a doctype's Custom DocPerm set, a
	...
	def test_a_doctype_with_no_custom_docperm_rows_is_untouched(self):
		...
		which only looks at doctypes with an existing Custom DocPerm row), so execute()
		must add nothing to it. 'Untouched' is proven two ways: the doctype's own Custom
	...
	def test_deleted_row_is_not_re_added_by_the_alert_hook(self):
		...
		ever reports and logs -- it must never write. Runs execute(), deletes one row it
		...
		self.assertTrue(inserted, "execute() inserted no permlevel 1 rows to delete")
