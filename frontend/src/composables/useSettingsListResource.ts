...
/**
 * The settings lists on screen, by doctype. A row action is handed the row and
 * nothing else, so a delete from a row menu has no other way to tell the list it
 * deleted from to refetch. frappe-ui exports nothing that reaches its own.
 */
 ...
 * re-exports neither it nor the path.
 */
...
/**
 * What a rendered list needs, and nothing about where the rows came from. Only a
 * doctype list has a `delete` or server-side filters, but a panel renders both
 * identically, and the narrow type is what lets SettingsListPanel hold either.
 */
...
export function useSettingsListResource<TRow = Record<string, any>>(
	...
	// number, so whichever response lands last wins. A Load More issued before a
	// search would overwrite the search results with unfiltered rows.
	...
	// `reload()` rewrites pageLength to the number of rows already loaded whenever
	// start > 0, so narrowing the list after Load More would ask for 26 rows in
	// one page. Going back to start 0 prevents it.
	...
	// start, and createListResource returns it before applying any new option, so
	// the panel would reopen filtered by a search its box no longer shows.
	...
	// it back. Leaving `start` advanced makes the next Load More ask for the page
	// after the one that failed, silently skipping it.
	const loadMore = () =>
		enqueue(async () => {
	...
	// frappe-ui's own delete handler refetches from its own onSuccess before this
	// one runs, keeping the current start. Rewinding afterwards is too late, so
	// rewind first and the refetch it starts is itself a page-one request.
...
export interface SettingsMethodResourceOptions {
	...
	method: string
	/**
	 * The doctype the rows belong to, as an invalidation key only, so that
	 * `reloadSettingsLists(doctype)` reaches this list. A delete from a row menu
	 * has to refetch without holding the resource behind it.
	 */
	doctype?: string
	/** Extra request params, read at call time, such as a filter's value. */
	params?: () => Record<string, unknown>
	/**
	 * The server's own page size. Load More is offered while a page comes back
	 * full, so this has to be what the method pages at.
	 */
	pageLength?: number
	auto?: boolean
}
...
/**
 * A settings list backed by a whitelisted method rather than a doctype, so the
 * paging is done here against `start`. No frappe-ui `cache` key: createResource
 * hands back the first instance for a key without rebinding its closures.
 */
export function useSettingsMethodResource<TRow = Record<string, any>>(
	...
	// in flight both resolve and both append. Each call takes a token, and a
	// superseded response is dropped along with its step of `start`.
	let requestToken = 0
	...
	const fetchPage = async (): Promise<void> => {
		...
		// exact-equality check hides Load More the moment the two disagree, and
		// stepping `start` by the constant would then skip rows.
		start.value = start.value + data.length
		hasNextPage.value = data.length >= pageLength
	}

	// Back to the first page, dropping what is on screen. The search term and the
	// filter both go to the server, so a match past page one is reachable without
	// pressing Load More first.
