	...
	>
		<template #default>
			...
			<!-- Narrow: one card per row, built from the same column metadata and fed
			     ...
			     The wrapper is the card shape's ListRows: frappe-ui puts the rows in a
			     `h-full overflow-y-auto` box of its own because the box it hands them
			     is `overflow-y-hidden`, and that class wins over any `overflow-y-auto`
			     passed in — Tailwind emits `hidden` last. Without one here the cards
			     below the fold are painted and unreachable. -->
			<div v-else class="h-full overflow-y-auto">
				<ul class="flex list-none flex-col gap-2">
					...
					>
						<!-- Outside the card element on purpose: the card may be a link or
						     a button, and a checkbox nested in either is not operable. -->
