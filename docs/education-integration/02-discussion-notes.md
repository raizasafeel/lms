...
- Both apps are moving to framework-aligned versioning (Education is already `17.0.0-dev`; Learning will jump from `2.45.x`). A major on both sides is the one moment breaking changes are cheap.
...
2. Both apps stay installable alone. Education owns the bridge and activates it only when Learning is installed. Learning ships a frozen contract (fields, server flags, a Python module) and never references Education.
...
| `Instructor.user` field, role mapping to Learning roles | Small |
...
| Custom fields, `after_app_install` handling, nightly reconciliation, tests | Medium |

What the Learning side owns: the contract (§4 of the design), the `medium` → `delivery_mode` rename, managed-batch locking and UI, profile privacy setting, docs.

## Decisions we need from the room

1. **Bridge ownership.** Agree Education owns the bridge code and Learning owns the contract. Alternative: a third "education_lms" app. We recommend against a third app; it adds a marketplace listing and a release train for a few hundred lines.
...
9. **Money.** Confirm: institution students never pay in Learning; fees stay in ERPNext. A school may still sell public courses through Learning's marketplace with Learning payments.
...
14. **Release.** `version-17` on both, compatibility Education 17 ↔ Learning 17 only, Frappe Cloud marketplace notes, install-order independence tested in CI. Who owns the joint upgrade guide?
...
- **Custom fields, not JSON.** Education's links to Learning doctypes are Custom Fields created at install/app-install time. Sites that had Learning installed before Education 17 depend on the `after_app_install` hook and the migration patch both being idempotent.
...
- **Testing.** Education CI currently installs Education alone. The bridge needs a second CI job that installs Learning too and runs the integration tests, in both install orders.

## Proposed timeline

| Week | Milestone |
|---|---|
| 1 | Both `version-17` branches cut; Learning contract PR opened; Education dead-code PR opened |
