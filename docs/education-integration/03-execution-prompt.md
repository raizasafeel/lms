...
### Step 6b — Batch sources hook (create a batch from another app's record)
1. Declare `lms_batch_sources` in `hooks.py` (empty list, with a comment): each entry is a dotted path to a module exposing three functions: `get_sources() -> list[dict(key, label)]`, `search_items(key, txt) -> list[dict(value, label, description)]`, `create_batch(key, value) -> str` (batch name). LMS never calls anything else from the provider.
2. In `lms/lms/integration.py` add whitelisted, privileged-only (`PRIVILEGED_ROLES`) wrappers: `get_batch_sources()`, `search_batch_source_items(key, txt)`, `create_batch_from_source(key, value)`. They iterate `frappe.get_hooks("lms_batch_sources")`, import each provider, and dispatch by `key`; unknown keys throw.
3. Frontend `frontend/src/pages/Forms/NewBatchForm.vue` (and the batches list "New" action if it does not open this form): when `get_batch_sources()` returns anything, show a "Create from…" control listing the sources; picking one shows a search box backed by `search_batch_source_items` and a "Create" button that calls `create_batch_from_source` and navigates to the new batch. With no sources registered the form is unchanged.
4. Unit test with a fake provider module registered through monkeypatched `frappe.get_hooks`; e2e test only that the control is absent when no provider exists.
...
Hook LMS declares for creating batches from another app's records: `lms_batch_sources` (a module with `get_sources()`, `search_items(key, txt)`, `create_batch(key, value)`), surfaced in LMS's New Batch form as "Create from…".
...
2. More native JSON fields: `Student Group.delivery_mode` Select `Classroom\nOnline\nBlended` default Classroom (insert after `group_based_on`); `Course Schedule.session_type` Select `In Person\nOnline\nAsynchronous` default In Person; `Student Attendance.session_type` (Select, read_only, `fetch_from: course_schedule.session_type`); `Assessment Plan.assessment_source` Select `Manual\nLMS Quiz\nLMS Assignment` default Manual, `Assessment Plan.attempt_policy` Select `Best attempt\nLatest attempt` default Best attempt; `Instructor.user` Link → User with `fetch_from: employee.user_id`, `fetch_if_empty: 1`; `Education Settings`: `enable_lms_integration` Check default 1, `live_class_attendance_threshold` Percent default 75, `auto_submit_live_class_attendance` Check default 0, `asynchronous_attendance_rule` Select `Any activity on the day\nCompleted scheduled lessons` default the first, `lms_profiles_configured` Check hidden, `migrate_legacy_lms_content` Check.
...
4. `Course Schedule.validate`: `room` is mandatory only when `session_type == "In Person"` (currently `reqd` in JSON; make it optional in JSON and validate in Python). `instructor` stays mandatory for all types.
...
- Batch source provider `lms_integration/batch_sources.py`, registered in `hooks.py` as `lms_batch_sources = ["education.education.lms_integration.batch_sources"]`: `get_sources()` returns `[{"key": "student_group", "label": "Student Group"}]`; `search_items` returns enabled Student Groups without `lms_batch` (label = group name, description = program / academic year / delivery mode); `create_batch("student_group", name)` sets the group's `delivery_mode` to `Blended` if it is `Classroom`, saves it (which runs `sync_group`), and returns `lms_batch`. Groups whose courses have no `lms_course` return the same validation error the desk shows.
...
- `mark_attendance_for_ended_sessions()` (hourly): Course Schedules with `session_type` in (`Online`, `Asynchronous`), ended more than 15 minutes ago (asynchronous: after 23:59 of the schedule date), and no Student Attendance rows yet for that schedule. For each student in the group: Online → participation = `get_live_class_participation(live_class)`; Present if `duration_seconds >= threshold% * scheduled_seconds` else Absent. Asynchronous → per `asynchronous_attendance_rule`: "Any activity on the day" = Present if the student has any `LMS Course Progress` (status Complete), `LMS Quiz Submission` or `LMS Assignment Submission` for one of the batch's courses with `creation` or `modified` on the schedule date (add `get_member_activity(member, courses, date)` to the LMS contract module if it is missing; otherwise query through `frappe.get_all`); "Completed scheduled lessons" = Present only if every `LMS Batch Timetable` row on that batch referencing a `Course Lesson` with that date has a Complete `LMS Course Progress` for the student. Then, for either type: respect `Student Leave Application` (status Leave) the same way `education.education.api.mark_attendance` does; reuse `make_attendance_records`; submit only when `auto_submit_live_class_attendance` is on. Skip schedules on or before `attendance_freeze_date`.
